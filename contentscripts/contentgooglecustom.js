...
let googleurls = []
    ...
    let TalalatokElements = []


window.addEventListener("load", function () {

document.querySelector(".gsc-search-button .gsc-search-button-v2").addEventListener("click"
, function () {

setTimeout(() => {
    
    console.log("document.querySelectorAll('.gsc-cursor-page'):", document.querySelectorAll('.gsc-cursor-page'));
    
    TalalatokElements.push(document.querySelectorAll('.gs-webResult'))
var interval = 2000; // how much time should the delay between two iterations be (in milliseconds)?
var promise = Promise.resolve();
document.querySelectorAll('.gsc-cursor-page').forEach(function (element) {
  promise = promise.then(function () {
 
    element.click()
setTimeout(() => {
    document.querySelectorAll('.gs-webResult').forEach(element => {
        
TalalatokElements.push(element)
    });
...
let TalalatokElements2=[]
promise.then(function () {
  console.log(TalalatokElements);

TalalatokElements.forEach(element => {
    
    
let cim = document.createElement("a")
 cim.innerText=element.querySelector(".gs-title").innerText
cim.href=element.querySelector(".gs-title").href


 TalalatokElements2.push([cim])


});

console.log("TalalatokElements2:", TalalatokElements2);
...
document.body.querySelector("#cse-header").insertAdjacentElement('afterbegin', document.createElement('table')).setAttribute("id", "example")
...
window.addEventListener("load", function () {
  ...
  let elementcolorregi="rgb(179, 179, 179)"
    document.querySelectorAll(".favicon").forEach(element => {
  
      console.log("element:", element);
      element.parentElement.parentElement.addEventListener('click', function (mezo) {
