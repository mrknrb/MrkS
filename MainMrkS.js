...
if (window.innerWidth < 860) {
  ...
  document.querySelector("#sessionbox").style = "display:none"
}
*/
let jelenlegitab = ""
...
function programstarter(htmlpath, cim, fileid) {
  ...
  tab.innerHTML = `<a class="tabbuttons" id="tabbutton${tabnumber}" href="#tab${tabnumber}">${cim}</a><a id="tabbezarobutton${tabnumber}" class="tabbezarobuttons" href="#tab${tabnumber}" style="width:20px; text-align: center;font-weight:900">×</a>`
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.sidebarid == sidebarid)
    if (request.kerestipus == "ujsidebartab") {
      if (request.tipus == "conceptmap") {
        programstarter("./programconceptmap.html", "Conceptmap", request.fileid)
      } else if (false) {
      } else {
      }
    }
